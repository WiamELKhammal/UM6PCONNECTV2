...
const Conversations = () => {
  ...
  const [activeTab, setActiveTab] = useState("chats"); // ✅ Default to chat list
  const [sidebarOpen, setSidebarOpen] = useState(false); // ✅ Track right sidebar state

  const handleSelectConversation = (contact) => {
    setSelectedConversation(contact);
    setSidebarOpen(false); // ✅ Close sidebar when switching chat
  };
  ...
  return (
    <Box sx={{ display: "flex", height: "100vh", backgroundColor: "#FFF" }}>
      ...
      {/* MIDDLE - Chat List or Archived Chats based on activeTab */}
      <Box>
        {activeTab === "chats" ? (
      ...
      {/* RIGHT - Chat Section (Dynamic Width) */}
      <Box
        sx={{
          display: "flex",
          flexDirection: "column",
          transition: "all 0.3s ease-in-out",
          width: sidebarOpen ? "calc(100% - 350px)" : "calc(100% - 80px)", // ✅ Adjust width smoothly
        }}
      >
        {!selectedConversation ? (
          ...
          <Box
            sx={{
              ...
              height: "100vh",
              overflow: "hidden",
              paddingRight: sidebarOpen ? "80px" : "0px", // ✅ Push ChatMessages Left
              transition: "padding-right 0.3s ease-in-out",
            }}
          >
            {/* ✅ Pass `onToggleSidebar` to ChatHeader */}
            <ChatHeader recipient={selectedConversation} onToggleSidebar={() => setSidebarOpen(!sidebarOpen)} />

            {/* Messages Container */}
            <Box sx={{ flex: 1, overflowY: "auto", padding: "1rem" }}>
              <ChatMessages messages={messages} recipient={selectedConversation} />
            </Box>
      ...
      {sidebarOpen && (
        <RightSidebar recipient={selectedConversation} onClose={() => setSidebarOpen(false)} />
      )}
    </Box>
  );
};
